...
class BatchImageCollateFunction(BaseCollateFunction):
    def __init__(
        ...
        cutmix_prob=0.0,
        cutmix_epochs=[0, 0],
        cutmix_context_pad=2.0,
        cutmix_max_attempts=30,
        cutmix_rotation_max=45,
    ) -> None:
        super().__init__()
        ...
        self.cutmix_epochs = cutmix_epochs
        self.cutmix_context_pad = cutmix_context_pad
        self.cutmix_max_attempts = cutmix_max_attempts
        self.cutmix_rotation_max = cutmix_rotation_max
        ...
        if self.cutmix_prob > 0:
            print("     ### Using class-balanced CutMix with Prob@{} in {} epochs ### ".format(cutmix_prob, cutmix_epochs))
            print(f'     ### CutMix -- context_pad@{cutmix_context_pad}x, max_attempts@{cutmix_max_attempts} ###')
    ...
    def apply_cutmix(self, images, targets):
        """Class-balanced CutMix for detection.

        For each image in the batch, probabilistically selects a source image
        that belongs to an under-represented class, cuts a context region around
        its marker, and pastes it onto the target image at a non-overlapping
        location.  Boxes are in normalised cxcywh format.
        """
        if not (self.cutmix_epochs[0] <= self.epoch < self.cutmix_epochs[-1]):
            return images, targets

        if self._cutmix_print_flag and self.epoch == self.cutmix_epochs[0]:
            print(f"     ### CutMix activated at epoch {self.epoch} ###")
            self._cutmix_print_flag = False
        if self.epoch == self.cutmix_epochs[-1]:
            print(f"     ### CutMix closed after epoch {self.epoch} ###")
            return images, targets
        ...
        # Build label → image-index map for this batch
        class_to_imgs = defaultdict(list)
        for i, t in enumerate(targets):
            for lbl in t['labels'].tolist():
                class_to_imgs[int(lbl)].append(i)

        if len(class_to_imgs) < 2:
            # Only one class present — still apply CutMix but without balancing
            all_classes = list(class_to_imgs.keys())
            minority_classes = set(all_classes)
        else:
            counts = {c: len(v) for c, v in class_to_imgs.items()}
            avg = sum(counts.values()) / len(counts)
            minority_classes = {c for c, n in counts.items() if n < avg}
            if not minority_classes:
                minority_classes = {min(counts, key=counts.get)}
        ...
        for i in range(batch_size):
            ...
            # Prefer sources from minority classes
            src_candidates = [j for c in minority_classes
                              for j in class_to_imgs.get(c, []) if j != i]
            if not src_candidates:
                src_candidates = [j for j in range(batch_size) if j != i]
            if not src_candidates:
                continue

            j = random.choice(src_candidates)

            src_boxes = targets[j]['boxes']   # [N, 4] normalised cxcywh
            src_labels = targets[j]['labels']
            if len(src_boxes) == 0:
                continue

            ann_idx = random.randint(0, len(src_boxes) - 1)
            cx, cy, bw, bh = src_boxes[ann_idx].tolist()
            src_label = src_labels[ann_idx]

            # Source crop with context padding
            pad = self.cutmix_context_pad
            cut_w = max(1, min(int(bw * pad * img_w) + 1, img_w))
            cut_h = max(1, min(int(bh * pad * img_h) + 1, img_h))

            x1s = max(0, int((cx - bw * pad / 2) * img_w))
            y1s = max(0, int((cy - bh * pad / 2) * img_h))
            x2s = min(img_w, x1s + cut_w)
            y2s = min(img_h, y1s + cut_h)
            cut_w = x2s - x1s
            cut_h = y2s - y1s
            ...
            # Find a non-overlapping destination in the target image
            tgt_boxes = targets[i]['boxes']
            placed = False

            for _ in range(self.cutmix_max_attempts):
                x1d = random.randint(0, max(0, img_w - cut_w))
                y1d = random.randint(0, max(0, img_h - cut_h))
                x2d = x1d + cut_w
                y2d = y1d + cut_h

                no_overlap = True
                for tb in tgt_boxes:
                    tx, ty, tw, th = tb.tolist()
                    tx1 = (tx - tw / 2) * img_w
                    ty1 = (ty - th / 2) * img_h
                    tx2 = (tx + tw / 2) * img_w
                    ty2 = (ty + th / 2) * img_h
                    if x1d < tx2 and x2d > tx1 and y1d < ty2 and y2d > ty1:
                        no_overlap = False
                        break
            ...
            # Optionally rotate patch before pasting
            patch = images[j, :, y1s:y2s, x1s:x2s]  # [C, cut_h, cut_w]
            if self.cutmix_rotation_max > 0:
                angle = random.uniform(-self.cutmix_rotation_max, self.cutmix_rotation_max)
                patch = VF.rotate(patch, angle, interpolation=InterpolationMode.BILINEAR, expand=False)
                # Update bbox to axis-aligned bounding box of rotated box
                import math
                rad = abs(math.radians(angle))
                rot_w = bw * abs(math.cos(rad)) + bh * abs(math.sin(rad))
                rot_h = bw * abs(math.sin(rad)) + bh * abs(math.cos(rad))
                bw, bh = min(rot_w, 1.0), min(rot_h, 1.0)

            # Paste crop
            updated_images[i, :, y1d:y2d, x1d:x2d] = patch

            # Recompute marker position in the destination frame
            new_cx = (x1d + cx * img_w - x1s) / img_w
            new_cy = (y1d + cy * img_h - y1s) / img_h
            half_w, half_h = bw / 2.0, bh / 2.0
            new_cx = float(max(half_w, min(1.0 - half_w, new_cx)))
            new_cy = float(max(half_h, min(1.0 - half_h, new_cy)))

            new_box = torch.tensor([new_cx, new_cy, bw, bh], dtype=src_boxes.dtype)
            updated_targets[i]['boxes'] = torch.cat(
                [updated_targets[i]['boxes'], new_box.unsqueeze(0)], dim=0
            )
            updated_targets[i]['labels'] = torch.cat(
                [updated_targets[i]['labels'], src_label.unsqueeze(0)], dim=0
            )
            if 'area' in updated_targets[i]:
                new_area = float(bw * img_w * bh * img_h)
                updated_targets[i]['area'] = torch.cat([
                    updated_targets[i]['area'],
                    torch.tensor([new_area], dtype=updated_targets[i]['area'].dtype)
                ], dim=0)
    ...
    def __call__(self, items):
        ...
        # Class-balanced CutMix
        images, targets = self.apply_cutmix(images, targets)
