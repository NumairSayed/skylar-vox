...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src, src_mask=None, pos_embed=None) -> torch.Tensor:
        ...
        # Use scaled_dot_product_attention (dispatches to Flash Attention when available)
        # to avoid materialising the full N×N attention matrix for large feature maps.
        B, N, C = q.shape
        ...
        q_ = F.linear(q, in_proj_weight[:C],     in_proj_bias[:C]     if in_proj_bias is not None else None)
        k_ = F.linear(k, in_proj_weight[C:2*C],  in_proj_bias[C:2*C]  if in_proj_bias is not None else None)
        v_ = F.linear(src, in_proj_weight[2*C:], in_proj_bias[2*C:]   if in_proj_bias is not None else None)

        q_ = q_.view(B, N, nhead, head_dim).transpose(1, 2)
        k_ = k_.view(B, N, nhead, head_dim).transpose(1, 2)
        v_ = v_.view(B, N, nhead, head_dim).transpose(1, 2)

        attn_out = F.scaled_dot_product_attention(q_, k_, v_, attn_mask=src_mask, dropout_p=0.0)
        attn_out = attn_out.transpose(1, 2).contiguous().view(B, N, C)
        src = F.linear(attn_out, self.self_attn.out_proj.weight, self.self_attn.out_proj.bias)
